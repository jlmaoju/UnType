...
录音时按数字键（1-8）即可切换，选择会自动记住。右键托盘图标 → **人格管理** 可自行增删改。

## 核心功能

- **AI 润色输出** — LLM 自动修正标点、语气词、语法和识别错误
- **语音编辑选中文字** — 选中文字，说出指令，LLM 执行修改
- **按键说话** — 按 F6 开始录音，再按结束；在任何应用中都能用
- **三语音识别后端** — 阿里云实时 API（默认）、在线 API、本地推理

## 使用体验细节

- **实时转录预览** — 使用阿里云实时 API，录音过程中即可看到识别文字，体验类似微信语音输入
- **录音时长显示** — 胶囊上显示录音时长（如 "1:23"），超过 5 分钟自动停止
- **音量可视化** — 录音时胶囊底部显示音量条，实时反馈麦克风状态
- **人格记忆** — 自动记住上次选择的人格面具，下次使用时自动选中
- **后悔药** — 注入后出现撤销菜单，支持恢复原文、重新生成或撤回编辑，没有倒计时压力
- **胶囊位置可调** — 固定模式（可拖动，位置会被记住）或跟随光标模式
- **快捷键录制** — 在设置中点击输入框后按下想要的键，即可自定义快捷键
- **日志记录** — 设置中可打开日志文件夹，方便问题排查
...
不想安装 Python？可以在 [Releases](https://github.com/jlmaoju/UnType/releases) 页面下载预编译的 `.exe` 文件，直接运行即可。
